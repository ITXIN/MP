...
protocol ScrollSegmentViewDelegate:NSObjectProtocol {
...
class ScrollSegmentView: UIView {
    ...
    var scrollSegmentDelegate:ScrollSegmentViewDelegate!
    ...
    func setupTitles() {
        for (index, title) in titles.enumerated() {
            ...
            /**
             这个设置每个的大小当滚动时
             */
            titlesWidthArray.append(size.width + 20)
...
extension ScrollSegmentView {
    ...
    // 手动滚动时需要提供动画效果
    func adjustUIWithProgress(_ progress: CGFloat,  oldIndex: Int, currentIndex: Int) {
    ...
    // 居中显示title
    func adjustTitleOffSetToCurrentIndex(_ currentIndex: Int) {
        ...
        if scrollSegmentDelegate != nil {
            scrollSegmentDelegate.currentIndex(currentIndex)
        }
        print("\(oldIndex) ---xxxx---- \(currentIndex)")
